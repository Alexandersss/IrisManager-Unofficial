/* 
	...
	(c) 2013 Alexander <www.openmod.it> ( Unofficial iris Version )
...
char * getlv2error(s32 error)
{
	switch(error) {
		case 0x00000000:
			return "Ok";
        case 0x80010001:
            return "La risorsa è temporaneamente non disponibile";
        case 0x80010002:
            return "Argomento Invalido";
        case 0x80010003:
            return "Funzione non implementata";
        case 0x80010004:
            return "Memoria allocata Fallita";
        case 0x80010005:
            return "Nessun processo";
        case 0x80010006:
            return "Nessun file o directory";
        case 0x80010007:
            return "Exec formato errato";
        case 0x80010008:
            return "Condizioni Deadlock";
		case 0x80010009:
            return "Operazione non permessa";
        case 0x8001000A:
            return "Dispositivo Occupato";
        case 0x8001000B:
            return "L'operazione è terminata";
        case 0x8001000C:
            return "L'Operazione è stata abortita";
        case 0x8001000D:
            return "Memoria di accesso non valida";
        case 0x80010012:
            return "Il file è una directory";
		case 0x80010013:
            return "Operazione cancellata";
        case 0x80010014:
            return "File esistente";
		case 0x80010015:
            return "Socket connesso";
		case 0x80010016:
            return "Socket non connesso";
        case 0x8001001B:
            return "Math arg out è un dominio di funzione";
		case 0x8001001C:
            return "Risultato matematico non rappresentabile";
        case 0x8001001D:
            return "Illegal multi-byte sequence in input";
        case 0x8001001E:
            return "Errore nella posizione del file";
		case 0x8001001F:
            return "Syscall è stata interrotta";
        case 0x80010020:
            return "File troppo grande";
        case 0x80010021:
            return "Troppi Link";
		case 0x80010022:
            return "Troppi file aperti nel sistema";
		case 0x80010023:
            return "Nessun spazio lasciato sul dispositivo";
		case 0x80010024:
            return "Non è una macchina da scrivere";
		case 0x80010025:
            return "Broken pipe";
		case 0x80010026:
            return "File System di sola lettura";
		case 0x80010027:
            return "Illegal seek";
        case 0x80010029:
            return "Permesso negato";
        case 0x8001002A:
            return "Descrittore di file non valido";
		case 0x8001002B:
            return "I/O error";
        case 0x8001002C:
            return "Troppi file aperti";
        case 0x8001002D:
            return "No è un dispositivo";
		case 0x8001002E:
            return "Non è una directory";
		case 0x8001002F:
            return "Non è un device o indirizzo";
        case 0x80010030:
            return "Cross-device link";
		case 0x80010031:
            return "Cercando di leggere un messaggio illeggibile";
		case 0x80010032:
            return "Connessione già in corso";
		case 0x80010033:
            return "Messaggio troppo lungo";
        case 0x80010034:
            return "Nome del file o percorso troppo lungo";
        case 0x80010035:
            return "Nessun record disponibile";
        case 0x80010036:
            return "La Directory non è vuota";
        case 0x80010037:
            return "Non supportato";
        case 0x80010039:
            return "Valore troppo grande per il tipo di dati da definire";
        case 0x8001003A:
            return "Filesystem non montato";
...
static int copy_archive_manager(char *path1, char *path2, sysFSDirent *ent, int nent, int sel, u64 free)
{
    ...
    if(sel>=0) {
        ...
        sprintf(temp_buffer, "Copiare file e cartelle selezionati\n\nfrom %s\n\nto %s?", path1, path2);
        if(DrawDialogYesNo(temp_buffer) == 1) {
            double_bar(cpy_str);
    ...
    if(size > free) {
        msgDialogAbort();
        usleep(100000);
        DrawDialogOK("Nessuno spazio per copiare file / cartelle");
    }
...
static int move_archive_manager(char *path1, char *path2, sysFSDirent *ent, int nent, int sel, u64 free)
{
    ...
    if(sel>=0) {
        ...
        sprintf(temp_buffer, "Spostare file e cartelle selezionati\n\nfrom %s\n\nto %s?", path1, path2);
        if(DrawDialogYesNo(temp_buffer) == 1) {
            double_bar(cpy_str);
    ...
    if(size > free) {
        msgDialogAbort();
        usleep(100000);
        DrawDialogOK("Nessuno spazio per spostare i file / cartelle");
    }
...
static int load_hex(s32 fd, u64 pos, void *buffer, u64 readed)
{
    ...
    if(fd == FD_LV1) {
        ...
        if(ret != 0) {
            sprintf(temp_buffer + 3072, "Errore di Lettura: 0x%08x\n\n%s", ret, getlv2error(ret));
...
static int save_hex(s32 fd, u64 pos, void *buffer, u64 readed)
{
    ...
    if(fd == FD_LV1) {
        ...
        if(ret != 0) {
            sprintf(temp_buffer + 3072, "Errore di Scrittura: 0x%08x\n\n%s", ret, getlv2error(ret));
            DrawDialogOK(temp_buffer + 3072);
        }
...
static char help2[]= {
    "X - Entra nell'opzione selezionata\n"
    "SU/GIU' - Seleziona Opzione\n"
    "\n"
    "TRIANGOLO - Esci\n"
    "SELECT - Opri/Chiudi Menu\n"
    "\n"
    "OpenMod --> ( Search in Google :) ): Archive Manager ( Traduzione italiana a cura di Alexander )\n"
};

static char help3[]= {
    "CROSS - Diminuire il nibble selezionato\n"
    "CERCHIO - Aumentare il nibble selezionato\n"
    "L1/R1 - Diminuisce / aumenta il byte selezionato (premendo a ripetizione automatica)\n"
    "\n"
    "QUADRATO - Annulla le modifiche al byte selezionato\n"
    "START - Annulla le attuali modifiche alle finestre\n"
    "START (Tendolo Premuto) - Apre / chiude questa finestra di aiuto\n"
    "\n"
    "TRIANGOLO - uscita dall'Hex Editor\n"
    "SELECT - Apre il menu di ricerca (vai a, trova ..)\n"
    "\n"
    "Su/Giù/Sinistra/Destra - Sposta il cursore"
    "\n"
    "L3/R3 - Va indietro /avanti tra le directory\n"
    "L2/R2 - Sposta  indietro /avanti i file dalle directory\n"
    "\n"
    "Nota speciale: Le modifiche nella finestra devono essere salvate per utilizzare alcune azioni con modifiche implicite"
    "nella finestra dell'editor. Premendo il tasto SELECT si può salvare o eliminare  cambiando schermatala (finestra) .\n"
};

static char help4[]= {
    "X - Imposta il numero esadecimale\n"
    "Su/Giù/Sinistra/Destra - Sposta il cursore della tastiera\n"
    "L2/R2 - Sposta indietro/avanti dalla finestra numero del hex editor\n"
    "QUADRATO - Cancella l'ultima digitazione\n"
    "\n"
    "CERCHIO - Va al percorso\n"
    "\n"
    "START (tenedolo premuto) - Apre / chiude questa finestra di aiuto\n"
    "TRIANGOLO - Uscita dall'Hex Editor\n"
    "SELECT - Chiudi la finestra\n"
    "\n"
    "Nota speciale: salta all'indirizzo di file assoluto"
};

static char help5[]= {
    "X - Imposta il numero esadecimale\n"
    "Su/Giù/Sinistra/Destra - Sposta il cursore della tastiera \n"
    "L2/R2 - Sposta indietro / avanti dalla finestra numero\n"
    "\n"
    "CERCHIO - Trova valori esadecimali nel file (in avanti)\n"
    "L1/R1 - diminuisce / aumenta il numero di byte da trovare\n"
    "\n"
    "START (tenendolo premuto) - Apre / chiude questa finestra di aiuto\n"
    "TRIANGOLO - Esce dall'Hex Editor\n"
    "SELECT - Chiude la finestra\n"
    "\n"
    "Nota speciale:. Trova i valori esadecimali nel file È possibile utilizzando L3/R3 da "
    "la finestra di editor esadecimale per trovare il risultato precedente / successivo dalla posizione corrente del file\n"
};
...
read_file:
    ...
    if(function_menu==1 || function_menu==2) {
        ...
        if(function_menu==1) DrawFormatString(px + 4, py + 4, "Vai all'indirizzo");
        else DrawFormatString(px + 4, py + 4, "Trova Hex Values");
    ...
    if(enable_menu && function_menu==0) {
        ...
        display_ttf_string(0, py, "Vai all'indirizzo", (enable_menu==1  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Trova Hex", (enable_menu==2  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Trova Stringa", (enable_menu==3  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Trova Stringa (no case)", (enable_menu==4 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Trova HEX da file", (enable_menu==5 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Segna Inizio", (enable_menu==6 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Segna Fine", (enable_menu==7 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Copy Mark Area", (enable_menu==8 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Incolla Dati copiati", (enable_menu==9 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Esci", (enable_menu==10 && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;
    ...
    if(new_pad & BUTTON_TRIANGLE) {
        ...
        if(locked && DrawDialogYesNo("Salvare i cambiamenti?") == 1) {
            save_hex(fd, pos, temp_buffer + 0x800, readed);
            memcpy(temp_buffer + 0xA00, temp_buffer + 0x800, 384);
            
        }

        if(DrawDialogYesNo("Esci dall'e Hex Editor?") == 1) {
            if(hex_mode == 1) {
                lv1_pos = pos;
            } else if(hex_mode == 2) {
                lv2_pos = pos;
    ...
    if((new_pad & BUTTON_R3) && find_len) {
        ...
        if(!locked) {
            ...
            if(finded == -1LL) {
                if(find_mode == 0) DrawDialogOKTimer("Hex String non trovata", 2000.0f);
    ...
    if((function_menu==1 || function_menu==2) && (new_pad & BUTTON_CIRCLE)) {
        switch(function_menu) {
            ...
            case 2:
                ...
                if(finded == -1LL) {
                    DrawDialogOK("Hex string nn trovata");
                } else {
                    pos = finded & ~15ULL;
                    e_y = 0;
                    e_x = (finded & 15) << 1;
        ...
        if(new_pad & BUTTON_CROSS) {
            ...
            switch(function_menu) {
                ...
                case 3:
                    ...
                    if(Get_OSK_String("Trova Stringa", (char *) find, 256)==0) {
                        u64 finded;
                        ...
                        if(find_len != 0) {
                            ...
                            if(finded == -1LL) {
                                DrawDialogOK("Stringa non trovata");
                            } else {
                                pos = finded & ~15ULL;
                                e_y = 0;
                                e_x = (finded & 15) << 1;
                ...
                case 4:
                    memset(find, 0, 512); find_len = 0;
                    if(Get_OSK_String("Trova Stringa(no case sensitive)", (char *) find, 256)==0) {
                        u64 finded;
                        ...
                        if(find_len != 0) {
                            ...
                            if(finded == -1LL) {
                                DrawDialogOK("Stringa (no case sensitive) non trovata");
                            } else {
                                pos = finded & ~15ULL;
                                e_y = 0;
                                e_x = (finded & 15) << 1;
                ...
                case 5:
                    if(copy_mem && copy_len) {
                        ...
                        if(find_len != 0) {
                            ...
                            if(finded == -1LL) {
                                DrawDialogOK("Hex string non trovata");
                            } else {
                                pos = finded & ~15ULL;
                                e_y = 0;
                                e_x = (finded & 15) << 1;
...
void archive_manager()
{
    ...
    if(set_menu2) {
        ...
        display_ttf_string(0, py, "Nuova Cartella", (set_menu2==1  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Rinomina", (set_menu2==2  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Copia", (set_menu2==3  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Muovi", (set_menu2==4  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Cancella", (set_menu2==5  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, !dev_rewrite ? "Monta /dev_rewrite" : "Smonta /dev_rewrite", (set_menu2==6  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;
        ...
        display_ttf_string(0, py, "Incolla in un Nuovo File", (set_menu2==11  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;

        display_ttf_string(0, py, "Esci", (set_menu2==12  && (frame & 16)) ? 0 : 0xffffffff, 0, 16, 24); py+= 24;
    ...
    if(new_pad & BUTTON_CROSS) {
        ...
        if(set_menu2==1) {// new folder
            ...
            sprintf(buffer1, "%s", "Nuovo"); 
            
            if(Get_OSK_String("Nuova cartella", buffer1, 256)==0) {

                 if(buffer1[0] == 0) {set_menu2 = 0;goto skip_menu2;}
                
                 sprintf(temp_buffer, "Crea Nuova Cartella %s\nto %s ?", buffer1, !archive_manager ? path1 : path2);
                 if(DrawDialogYesNo(temp_buffer) == 1) {
                     ...
                     if(ret<0) {
                        sprintf(temp_buffer, "Errore Nuova Cartella: 0x%08x\n\n%s", ret, getlv2error(ret));
            ...
            if(Get_OSK_String("Rinomina", buffer1, 256)==0) {
                
                 sprintf(temp_buffer, "Rinomina %s\nto %s ?", !archive_manager ? entries1[sel1].d_name : entries2[sel2].d_name,
                     buffer1);
                 if(DrawDialogYesNo(temp_buffer) == 1) {
                     ...
                     if(ret<0) {
                        sprintf(temp_buffer, "Errore di Rinominazione: 0x%08x\n\n%s", ret, getlv2error(ret));
                        DrawDialogOK(temp_buffer);
                     }
            ...
            if(test_mark_flags(entries, nentries, &files)) {// multiple
                ...
                sprintf(temp_buffer, "Cancellare file e cartelle sezionati?\n\n(%i) Items", files);
                if(DrawDialogYesNo(temp_buffer) == 1) {
                              
                    single_bar("Cancellazione...");
                    ...
                    if(ret<0) {
                        sprintf(temp_buffer, "Errore di Eliminazione: 0x%08x\n\n%s", ret, getlv2error(ret));
                        DrawDialogOK(temp_buffer);    
                    }
                ...
                sprintf(temp_buffer, "Elimina %s?", entries[sel].d_name);
            ...
            if(!archive_manager) {
             ...
             if(ret<0) {
                sprintf(temp_buffer, "Errore Copia: 0x%08x\n\n%s", ret, getlv2error(ret));
                DrawDialogOK(temp_buffer);
             }
            ...
            if(ret<0) {
                sprintf(temp_buffer, "Errore in LV2 dump: 0x%08x\n\n%s", ret, getlv2error(ret));
                DrawDialogOK(temp_buffer);
             }
            ...
            if(ret<0) {
                sprintf(temp_buffer, "Errore in LV1 dump: 0x%08x\n\n%s", ret, getlv2error(ret));
                DrawDialogOK(temp_buffer);
             }
            ...
            sprintf(buffer1, "%s", "Nuovo File"); 
            
            if(Get_OSK_String("Copia in Nuovo File", buffer1, 256)==0) {

                 if(buffer1[0] == 0) {DrawDialogOKTimer("Invalid filename", 2000.0f);set_menu2 = 0;goto skip_menu2;}
                
                 sprintf(temp_buffer, "Crea Nuovo File %s.bin\nto %s ?", buffer1, !archive_manager ? path1 : path2);
                 if(DrawDialogYesNo(temp_buffer) == 1) {
                     ...
                     if(ret<0) {
                        sprintf(temp_buffer, "Errore Nuovo File: 0x%08x\n\n%s", ret, getlv2error(ret));
                        DrawDialogOK(temp_buffer);
                     } else {
                        sprintf(temp_buffer, "Scrivi %d bytes nella posizione corrente", copy_len);
    ...
    if(new_pad & BUTTON_TRIANGLE) {
        if(DrawDialogYesNo("Uscire dall' Archive Manager?") == 1) {
            break;
