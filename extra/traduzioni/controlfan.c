...
int test_controlfan_compatibility() 
{
    if(firmware == 0x341C || firmware == 0x355C || firmware == 0x355D|| firmware == 0x421C || firmware == 0x421D ||
         firmware == 0x430C || firmware == 0x430D ||  firmware == 0x431C || firmware == 0x440C || firmware == 0x441C || firmware == 0x441D || firmware == 0x446C || firmware == 0x446D) return 1;
...
skip_the_load:
    ...
    free(addr);
    return ret;void set_usleep_sm_main(u32 us);
}

static u32 speed_table[8] = {
    0x6F,     // sm_shutdown / manual mode
    0x54,     // < temp_control0
    0x59,     // temp_control0 => temp_control1
    0x5e,     // temp_control0 <= temp_control1
    0x64,     // >= temp_control1
    0x6a,     // >= temp_control2
    0x70,     // >= temp_control3
    0xA0     // >= temp_control4
...
static int fan_mode = 1; //syscon mode
static int cur_fan_mode = 1; //syscon mode
...
void load_controlfan_config() 
{
    ...
    if(!mem || n != file_size) {if(mem) free(mem); set_fan_mode(1); return;}
...
void draw_controlfan_options()
{
    ...
    if(cur_fan_mode == 0) {
        if(sm_present) 
            DrawFormatString(x2, y - 0, "Attuale: #S. Manager");
        else
            DrawFormatString(x2, y - 0, "Attuale: #Payload");
    }
    else if(cur_fan_mode == 1)
        DrawFormatString(x2, y - 0, "Attuale: #SYSCON");
    else if(cur_fan_mode == 2)
        DrawFormatString(x2, y - 0, "Attuale: #By User");
    ...
    y2+= 48;
    x2=DrawButton1_UTF8(x + 32, y2, 212, "User/Velocità spegnimento", (flash && select_option == 1)) + 8;
    ...
    y2+= 48;
    sprintf(temp_buffer, " < %2i ºC Velocità ", temp_control[0]);
    ...
    y2+= 48;
    sprintf(temp_buffer, " < %2i ºC Velocità ", temp_control[1]);
    ...
    y2+= 48;
    sprintf(temp_buffer, " >= %2i ºC Velocità ", temp_control[2]);
    ...
    if(!wakeup_time) sprintf(temp_buffer, " Disabilitato ");
    else sprintf(temp_buffer, " %i s ", wakeup_time * 10);
    x2 = DrawButton2_UTF8(x2, y2, 0, temp_buffer, 1) + 8;

    y2+= 48;
    sprintf(temp_buffer, " >= %2i ºC Velocità ", temp_control[3]);
    ...
    SetFontColor(0xffffffff, 0x00000000);
    SetFontSize(16, 20);
    DrawFormatString(x2, y2 + 8, "Usa L1/R1 o X/O");
    DrawFormatString(x2, y2 + 32, "per cambiare i valori");
    DrawFormatString(x2, y2 + 56, "SX/DX per spostarsi tra");
    DrawFormatString(x2, y2 + 80, "le colonne");

    y2+= 48;
    sprintf(temp_buffer, " >= %2i ºC Velocità ", temp_control[4]);
    x2=DrawButton1_UTF8(x + 32, y2, 212, temp_buffer, (flash && select_option == 8)) + 8;
    sprintf(temp_buffer, " %02X ", speed_table[7]);
    x2 = DrawButton2_UTF8(x2, y2, 0, temp_buffer, 1) + 8;

    x2=DrawButton1_UTF8(x + 360, y2, 212, "Imposta Settaggi", (flash && select_option == 16)) + 8;
